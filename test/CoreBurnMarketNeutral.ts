...
describe("Core: burn market neutral positions", () => {
  ...
  it(`should burn a single market neutral position and return the initial margin`, async () => {
    ...
    const tx2 = await core["burnMarketNeutral(address[2],(uint256,uint256,uint256[],address,address,address))"]([shortPositionAddress, longPositionAddress], optionCall)
    await tx2.wait()
  ...
  it(`should burn all the market neutral positions of the caller and return all their margins`, async () => {
    ...
    const shortPositionERC20 = <OpiumPositionToken>await ethers.getContractAt("OpiumPositionToken", shortPositionAddress)
    const longPositionERC20 = <OpiumPositionToken>await ethers.getContractAt("OpiumPositionToken", longPositionAddress)
    const secondShortPositionERC20 = <OpiumPositionToken>await ethers.getContractAt("OpiumPositionToken", secondShortPositionAddress)
    const secondLongPositionERC20 = <OpiumPositionToken>await ethers.getContractAt("OpiumPositionToken", secondLongPositionAddress)
    ...
    //@ts-ignore
    const tx3 = await core["burnMarketNeutral(address[2][],(uint256,uint256,uint256[],address,address,address)[])"]([[shortPositionAddress, longPositionAddress], [secondShortPositionAddress, secondLongPositionAddress]], [optionCall, secondOptionCall])
    await tx3.wait()

    const marketNeutralPartyLongBalanceAfter = await longPositionERC20.balanceOf(marketNeutralParty.address)
    const marketNeutralPartysShortBalanceAfter = await shortPositionERC20.balanceOf(marketNeutralParty.address)
    const marketNeutralPartySecondLongBalanceAfter = await secondLongPositionERC20.balanceOf(marketNeutralParty.address)
    const marketNeutralPartySecondShortBalanceAfter = await secondShortPositionERC20.balanceOf(marketNeutralParty.address)

    const tokenBalanceAfterBurn = await testToken. balanceOf(marketNeutralParty.address);
    console.log(`token balance after all market neutral positions have been burnt: ${tokenBalanceAfterBurn.toString()}`)
