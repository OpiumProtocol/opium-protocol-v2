...
abstract contract UsingRegistry is Initializable, UsingRegistryErrors {
    // Emitted when registry instance is set
    event LogRegistrySet(address _registry);

    // Instance of Opium.Registry contract
    IRegistry internal registry;

    /// @notice Defines registry instance and emits appropriate event
    function __UsingRegistry__init__(address _registry) initializer internal {
        registry = IRegistry(_registry);
        emit LogRegistrySet(_registry);
    }
